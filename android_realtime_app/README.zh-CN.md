...
- 支持运行内置离线示例，或选择论文格式 ACC/GYRO `.txt` 文件进行离线识别。
...
6. 在识别页面可运行内置离线示例，或选择论文格式 ACC/GYRO `.txt` 文件，使用同一套论文推理流程进行离线识别。
...
开源版不随仓库分发离线传感器样例。需要演示 App 端离线推理路径时，请通过文件选择器载入已获授权的论文格式 ACC/GYRO TXT 文件。
